...
class SendScheduledSMS extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:send-scheduled-sms {id} {phone} {content}';
    ...
    public function handle()
    {
        ...
        $id = $this->argument('id');
        //list($sms_id, $sms_cnt, $cost, $balance) = send_sms(89026790011, 'content', 1);
        /*$send = new SmsController;
        $send -> send($phone, $content);*/
        /*User::create([
             'name' => $phone,
            'email' =>  $content.'1@1qq23.ru '.$id,
            'password' => 12345678,

        ]);*/
        $date = Carbon::now('Asia/Omsk');
        Sent::create([
            'newsletter_id' =>  $id,
            'created_at' => $date,
        ]);
        ...
        if (!defined('SMSC_LOGIN')) {
            define('SMSC_LOGIN', 'ivankefir');
        }
       // пароль
        if (!defined('SMSC_PASSWORD')) {
            define('SMSC_PASSWORD', 'Avbk.36kgTYGDLq');
        }

        if (!defined('SMSC_POST')) {
            define('SMSC_POST', 1);
        }
        // использовать метод POST
        if (!defined('SMSC_HTTPS')) {
            define('SMSC_HTTPS', 0);
        }
        // использовать HTTPS протокол
        if (!defined('SMSC_CHARSET')) {
            define('SMSC_CHARSET', 'utf-8');
        }

        if (!defined('SMSC_DEBUG')) {
            define('SMSC_DEBUG', 0);
        }
        // флаг отладки
        ...
        function get_sms_cost($phones, $message, $translit = 0, $format = 0, $sender = "Andrei", $query = "")
        {
            static $formats = array(1 => "flash=1", "push=1", "hlr=1", "bin=1", "bin=2", "ping=1", "mms=1", "mail=1", "call=1", "viber=1", "soc=1");
